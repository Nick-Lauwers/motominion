...
#  searchkick word_start: [:listing_name, :city], locations: [:location]
  ...
  scope :with_year_gte, lambda { |ref_year|
    where('vehicles.year >= ? AND vehicles.year = ?', ref_year, nil)
  }
  ...
  scope :with_actual_price_lte, lambda { |ref_price|
    where('vehicles.actual_price <= ? AND vehicles.actual_price = ?', ref_price, nil)
  }
  ...
  scope :with_mileage_numeric_lte, lambda { |ref_mileage|
    where('vehicles.mileage_numeric <= ? AND vehicles.engine_size = ?', ref_mileage, nil)
  }
  ...
  scope :with_engine_size_gte, lambda { |ref_engine_size|
    where('vehicles.engine_size >= ? AND vehicles.engine_size = ?', ref_engine_size, nil)
  }
