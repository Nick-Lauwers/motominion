...
CSV.foreach(Rails.root.join("vehicles.csv"), headers: true) do |row|
  Vehicle.create! do |vehicle|
    ...
    vehicle.color                = row[2]
    vehicle.engine_type          = row[3]
    vehicle.fuel_type            = row[4]
    vehicle.vin                  = row[5]
    vehicle.listing_name         = row[6]
    vehicle.street_address       = row[7]
    vehicle.apartment            = row[8]
    vehicle.city                 = row[9]
    vehicle.state                = row[10]
    vehicle.year                 = row[11]
    vehicle.price                = row[12]
    vehicle.mileage              = row[13]
    vehicle.engine_size          = row[14]
    vehicle.summary              = row[15]
    vehicle.sellers_notes        = row[16]
    vehicle.is_cruise_control    = row[17]
    vehicle.is_am_fm             = row[18]
    vehicle.is_cb_radio          = row[19]
    vehicle.is_navigation_system = row[20]
    vehicle.is_heated_seats      = row[21]
    vehicle.is_heated_hand_grips = row[22]
    vehicle.is_alarm_system      = row[23]
    vehicle.is_saddlebags        = row[24]
    vehicle.is_trunk             = row[25]
    vehicle.is_tow_hitch         = row[26]
    vehicle.is_cycle_cover       = row[27]
    vehicle.user_id              = row[28]
    vehicle.vehicle_make_id      = row[29]
    vehicle.vehicle_model_id     = row[30]
    vehicle.latitude             = row[31]
    vehicle.longitude            = row[32]
    vehicle.bumped_at            = row[33]
    vehicle.posted_at            = row[34]
  end
end
